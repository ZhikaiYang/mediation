...
```{r}
mgmi <- fread("largedata/mediator_out_microbiome_fixed.txt", header=TRUE, data.table=FALSE)

## freq of the mediators
mgmit <- data.frame(table(mgmi$id))
mgmit <- mgmit[order(mgmit$Freq, decreasing = T),]
hist(log10(mgmit$Freq), main = "Distribution of log10(Deteced Freq of Mediators)", xlab = "log10(Deteced Freq of Mediators)" )
```
...
### select the top 4 with Freq >2
idx <- which(names(rna) %in% mgmit$Var1[1:5])
myr <- rna[, c(1:9, idx)]

write.table(myr, "largedata/top5_microbiome_mediators.csv", sep=",", row.names = FALSE, quote=FALSE)
```
...
top5_mi <- fread("largedata/top5_microbiome_mediators.csv", header=TRUE, data.table=FALSE)
table(top5_mi$TissueWODate)
```
...
```
#Compute average expression level by tissue # microbiome
```{r}
library(tidyr)
library(plyr)

sub <- top5_mi[, c(3, 10:14)]
df <- gather(sub, key="gene", value="fpm", 2:ncol(sub))
dim(subset(df, TissueWODate %in% "LMAD" & gene %in% "AC148152.3_FG008"))
# 210   3
...
pdf("graphs/top5_microbiome_fpm_by_tissue.pdf", width = 8, height= 6)
p
dev.off()
```

...
pvm_h2 = merge(h2_new_s, pvm_stat, by.x = "trait", by.y = "trait")
...
```{r}
pvm_s = subset(pvm, category == "Metabolite")
...
model=lm(pvm_s$pmed.pure~pvm_s$tissue )
ANOVA=aov(model)

# Tukey test to study each pair of treatment :
TUKEY <- TukeyHSD(x=ANOVA, 'pvm_s$tissue', conf.level=0.95)

library(multcompView)
labels<-generate_label_df(TUKEY , "pvm_s$tissue")#generate labels using function

names(labels)<-c('Letters','tissue')#rename columns for merging
pvm_s = pvm_s[,c(11,2)]
yvalue<-aggregate(.~tissue, data=pvm_s, mean)# obtain letter position for y axis using means

final<-merge(labels,yvalue) #merge dataframes


library(ggplot2)
ggplot(pvm_s, aes(x = tissue, y = pmed.pure)) +
  geom_blank() +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(x = 'Tissue', y = 'PVM(Proportion of Variance Mediated') +
  ggtitle(expression(atop(bold("Tukey Test of PVM in 7 Tissues")))) +
  ...
  geom_text(data = final, aes(x = tissue, y = pmed.pure, label = Letters),vjust=-3.5,hjust=-.5) +
...
id1= which(pvm_s$category == "Flowering Time")
id2 = which(pvm_s$trait == "PlantHeight")
pvm_s = pvm_s[c(id1),]
...
pvm_s$tissue <- factor(pvm_s$tissue, levels = c( "L3Tip", "GRoot", "GShoot", "L3Base",  "LMAD", "LMAN","Kern"),
                    labels =c( "Leaf 3 tip", "Germinating Root", "Germinating Shoot", "Leaf 3 base",  "Adult leaf during the day", "Adult leaf at night","Kernels after pollination"))
pvm_s$category = as.factor(pvm_s$category)

model=lm(pvm_s$pmed.pure~pvm_s$tissue )

pair_c=  pairs(emmeans(model, "tissue"))
...
pvm = fread("largedata/pvm_1897rows_microbiome.csv")
...
pdf("graphs/pvm_box_microbiome_tissue.pdf", width = 9, height= 6)
p_tissue
dev.off()


#################subset out other and metabolite, not bi###
pvm = subset(pvm, !(category %in% c("other", "Metabolite")))

pvm$category <- factor(pvm$category, levels = c( "Seed Traits", "Female Inflorescence", "Leaf Morphology", "Development", "Flowering Time", "Male Inflorescence"),
                    labels =c( "Seed Traits", "Female Inflorescence", "Leaf Morphology", "Development", "Flowering Time", "Male Inflorescence"))
...
pdf("graphs/pvm_box_ags_tissue.pdf", width = 9, height= 6)
p_tissue
dev.off()
...
```{r}
pvm = subset(pvm, category == "Metabolite")
...
model=lm(pvm$pmed.pure~pvm$tissue )
ANOVA=aov(model)

# Tukey test to study each pair of treatment :
TUKEY <- TukeyHSD(x=ANOVA, 'pvm$tissue', conf.level=0.95)

library(multcompView)
labels<-generate_label_df(TUKEY , "pvm$tissue")#generate labels using function

names(labels)<-c('Letters','tissue')#rename columns for merging
pvm = pvm[,c(11,2)]
yvalue<-aggregate(.~tissue, data=pvm, mean)# obtain letter position for y axis using means

final<-merge(labels,yvalue) #merge dataframes


library(ggplot2)
ggplot(pvm, aes(x = tissue, y = pmed.pure)) +
  geom_blank() +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(x = 'Tissue', y = 'PVM(Proportion of Variance Mediated') +
...
id1= which(pvm$category == "Flowering Time")
id2 = which(pvm$trait == "PlantHeight")
pvm = pvm[c(id1),]
...
pvm$tissue <- factor(pvm$tissue, levels = c( "L3Tip", "GRoot", "GShoot", "L3Base",  "LMAD", "LMAN","Kern"),
                    labels =c( "Leaf 3 tip", "Germinating Root", "Germinating Shoot", "Leaf 3 base",  "Adult leaf during the day", "Adult leaf at night","Kernels after pollination"))
pvm$category = as.factor(pvm$category)

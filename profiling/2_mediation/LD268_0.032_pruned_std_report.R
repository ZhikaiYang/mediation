...
if(length(id.fixed) >= 1){
  fwrite(Xcolnames[id.fixed], 
         "/common/jyanglab/zhikaiyang/projects/mediation-analysis/largedata/code2020/id.fixed_gddsilk_0.032_pruned_ld268.csv", 
         sep=",", row.names=FALSE, quote=FALSE)}
directsnps.fixed = reportDirectSNPs(mod.fixed,tests.fixed,pval.cut)
if(length(names(directsnps.fixed$coef.outcome))>=1){
  fwrite(as.list(names(directsnps.fixed$coef.outcome)), 
         "/common/jyanglab/zhikaiyang/projects/mediation-analysis/largedata/code2020/id.fixed_gddsilk_0.032_pruned_ld268_dsnps.csv", 
         sep=",", row.names=FALSE, quote=FALSE)}

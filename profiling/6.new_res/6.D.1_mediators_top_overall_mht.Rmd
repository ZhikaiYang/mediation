...
### new result for microbiome
library(data.table)
mi <- fread("largedata/mediator_46652rows_microbiome.txt")
# res.eq
...
rm(x)
#>>>>>>> bb5f4e98862ac2307e30185f5780f2610a05cffa
```




#Find the top mediators for microbiome
#stdN
```{r}
library(plyr)
library("data.table")
...
pdf("graphs/mht_num_med_traits_microbiome_stdN.pdf", width = 15, height= 5)
plot_mht(inputdf=p2, hdf=NULL, col2plot = "V1", jmph = FALSE, pl = TRUE,
  cl_file = "data/chr_length_B73v4.csv", cex = 5,
  pch = 16, col = rep(c("slateblue", "cyan4"), 5), GAP = 5e+06,
  yaxis = NULL, ylab="Number of mediated traits")
...
m2 <- subset(mi, treatment %in% "lowN" & method %in% "res.fixed" & padj <= 0.05)
m2$type <- "Microbiome"
...
pdf("graphs/mht_num_med_traits_microbiome_lowN.pdf", width = 15, height= 5)
plot_mht(inputdf=p2, hdf=NULL, col2plot = "V1", jmph = FALSE, pl = TRUE,
  cl_file = "data/chr_length_B73v4.csv", cex = 5,
  pch = 16, col = rep(c("slateblue", "cyan4"), 5), GAP = 5e+06,
  yaxis = NULL, ylab="Number of mediated traits")
...
```{r}
mi_lowN = subset(mi, id %in% p2$id)
mi_stdN = subset(mi, id %in% p2$id)
mi_lowN = subset(mi_lowN, padj <= 0.05 & treatment == "lowN" & method == "res.fixed")
mi_stdN = subset(mi_stdN, padj <= 0.05 & treatment == "stdN" & method == "res.fixed")
mi_stdN = merge(mi_stdN, p2, by= "id")
mi_lowN = merge(mi_lowN, p2, by= "id")
colnames(mi_stdN)[c(12,13)] = c("Freq", "id_v4")
fwrite(mi_stdN, "largedata/mediators_microbiome_stdN.txt", sep="\t", row.names = F, quote = F)

colnames(mi_lowN)[c(12,13)] = c("Freq", "id_v4")
fwrite(mi_lowN, "largedata/mediators_microbiome_lowN.txt", sep="\t", row.names = F, quote = F)

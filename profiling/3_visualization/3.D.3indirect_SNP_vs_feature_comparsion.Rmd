...
#####change by features########################################################################################
isnps_f_gene <- fread("largedata/isnps_vs_feature/isnps_in_g.txt")
#####change by features########################################################################################


isnps_f_gene_table <- as.data.frame(table(isnps_f_gene$snps_for_medi)) 
isnps_f_gene_table$Var1 = as.character(isnps_f_gene_table$Var1)
isnps = merge(isnps, isnps_f_gene_table, by.x = "snps_for_medi", by.y = "Var1", sort = F, all.x = T)
...
for (trait in i_trait_eq) {
  ...
  for (tissue in i_tissue_eq) {
    ...
    if (length(t) > 0) {
      ...
      f_gene_overlap_eq = rbind(f_gene_overlap_eq, c(trait, tissue, o_ratio, length(which(tem$Freq != 0)), nrow(tem) ))
    }
...
for (trait in i_trait_fixed) {
  ...
  for (tissue in i_tissue_fixed) {
    ...
    if (length(t) > 0) {
      tem = isnps_fixed[t, ]
      o_ratio = length(which(tem$Freq != 0))/nrow(tem)
      f_gene_overlap_fixed = rbind(f_gene_overlap_fixed, c(trait, tissue, o_ratio, length(which(tem$Freq != 0)), nrow(tem) ))
    }
...
f_gene_overlap_eq = as.data.frame(f_gene_overlap_eq)
colnames(f_gene_overlap_eq) = c("trait", "tissue", "o_ratio", "num_o", "N_isnps")
f_gene_overlap_fixed = as.data.frame(f_gene_overlap_fixed)
colnames(f_gene_overlap_fixed) = c("trait", "tissue", "o_ratio", "num_o", "N_isnps")

f_gene_overlap_eq = merge(f_gene_overlap_eq, trait_category, by = "trait", all.x = T, sort = F)
f_gene_overlap_fixed = merge(f_gene_overlap_fixed, trait_category, by = "trait", all.x = T, sort = F)
...
```{r}
fsize=18
f_gene_overlap_eq$tissue <- factor(f_gene_overlap_eq$tissue, levels = c( "GRoot", "GShoot", "L3Base", "L3Tip", "LMAD", "LMAN","Kern"),
...
f_gene_overlap_eq$o_ratio = as.numeric(f_gene_overlap_eq$o_ratio)

library(ggplot2)

#df0 <- subset(df, cat != "1")
pall <- ggplot(f_gene_overlap_eq, aes(x=tissue, y=o_ratio, fill=category)) +
    ...
    xlab("") +
    ylab("Overlap ratio with f_gene") +
    ggtitle("") +
    theme_classic() +
    labs(fill = "") +
    scale_y_continuous(limits = c(-0.01, max(f_gene_overlap_eq$o_ratio))) +
...
pdf("graphs/overlap_ratio_maxscale_f_gene_all_traits_eq.pdf", width = 9, height= 6)
pall
dev.off()
...
```{r}
fsize=18
f_gene_overlap_fixed$tissue <- factor(f_gene_overlap_fixed$tissue, levels = c( "GRoot", "GShoot", "L3Base", "L3Tip", "LMAD", "LMAN","Kern"),
                    labels =c( "Germinating Root", "Germinating Shoot", "Leaf 3 base", "Leaf 3 tip", "Adult leaf during the day", "Adult leaf at night","Kernels after pollination"))
f_gene_overlap_fixed$o_ratio = as.numeric(f_gene_overlap_fixed$o_ratio)

library(ggplot2)

#df0 <- subset(df, cat != "1")
pall <- ggplot(f_gene_overlap_fixed, aes(x=tissue, y=o_ratio, fill=category)) +
    #geom_bar(stat="identity", position=position_dodge()) +
    geom_boxplot(outlier.colour="black", outlier.shape=16, 
             outlier.size=1, notch=FALSE) +
    # scale_fill_manual(values=c("#69b3a2", "#cdc0b0")) +
    ...
    theme_classic() +
    labs(fill = "") +
    scale_y_continuous(limits = c(-0.01, max(f_gene_overlap_fixed$o_ratio))) +
    theme(plot.title = element_text(size=20, face = "bold"), 
          axis.text=element_text(size=16, face="bold"),
          strip.text.x = element_text(size = 16, face = "bold"),
          axis.title=element_text(size=fsize, face="bold"),
...
pdf("graphs/overlap_ratio_maxscale_f_gene_all_traits_fixed.pdf", width = 9, height= 6)
pall
dev.off()
...
#p7msnps_int327ear: 8181 0.01174508
#p7msnps_int34ear: 2631 0.003777204
#p7msnps_f_gene: 52416 0.06810014
...
#####change by features########################################################################################
f_gene_overlap_eq$p_f_null = 0.06878224
 
f_gene_overlap_fixed$p_f_null = 0.06878224

#####change by features########################################################################################



f_gene_overlap_eq$num_o = as.numeric(f_gene_overlap_eq$num_o)
f_gene_overlap_fixed$num_o = as.numeric(f_gene_overlap_fixed$num_o)
f_gene_overlap_eq$N_isnps = as.numeric(f_gene_overlap_eq$N_isnps)
f_gene_overlap_fixed$N_isnps = as.numeric(f_gene_overlap_fixed$N_isnps)

f_gene_overlap_eq$p_val_chi = 0.0
f_gene_overlap_eq$p_val_chi_adj = 0.0
f_gene_overlap_eq$statistic_chi = 0.0
for (i in 1:nrow(f_gene_overlap_eq)) {
  t = chisq.test(c(f_gene_overlap_eq$num_o[i], f_gene_overlap_eq$N_isnps[i]-f_gene_overlap_eq$num_o[i]),p=c(f_gene_overlap_eq$p_f_null[i], 1-f_gene_overlap_eq$p_f_null[i]))
  f_gene_overlap_eq$statistic_chi[i] = t$statistic
  f_gene_overlap_eq$p_val_chi[i] = t$p.value
  f_gene_overlap_eq$p_val_chi_adj[i] = p.adjust(f_gene_overlap_eq$p_val_chi[i], method = "fdr", n= nrow(f_gene_overlap_eq))
}

f_gene_overlap_fixed$p_val_chi = 0.0
f_gene_overlap_fixed$p_val_chi_adj = 0.0
f_gene_overlap_fixed$statistic_chi = 0.0
for (i in 1:nrow(f_gene_overlap_fixed)) {
  t = chisq.test(c(f_gene_overlap_fixed$num_o[i], f_gene_overlap_fixed$N_isnps[i]-f_gene_overlap_fixed$num_o[i]),p=c(f_gene_overlap_fixed$p_f_null[i], 1-f_gene_overlap_fixed$p_f_null[i]))
  f_gene_overlap_fixed$statistic_chi[i] = t$statistic
  f_gene_overlap_fixed$p_val_chi[i] = t$p.value
  f_gene_overlap_fixed$p_val_chi_adj[i] = p.adjust(f_gene_overlap_fixed$p_val_chi[i], method = "fdr", n= nrow(f_gene_overlap_fixed))
}

length(which(f_gene_overlap_eq$p_val_chi_adj > 0.05))
length(which(f_gene_overlap_fixed$p_val_chi_adj > 0.05))



hist(f_gene_overlap_eq$p_val_chi_adj[- which(f_gene_overlap_eq$p_val_chi_adj > 0.05)], main = "Distribution of adjusted p-value by FDR (Medfix.0.5)", xlab = "adjusted p-value")

hist(f_gene_overlap_fixed$p_val_chi_adj[- which(f_gene_overlap_fixed$p_val_chi_adj > 0.05)], main = "Distribution of adjusted p-value by FDR (Medfix.bic)", xlab = "adjusted p-value")

write.table(f_gene_overlap_eq, "largedata/isnps_vs_feature/f_gene_overlap_eq.csv", sep=",", row.names=FALSE, quote=FALSE)
write.table(f_gene_overlap_fixed, "largedata/isnps_vs_feature/f_gene_overlap_fixed.csv", sep=",", row.names=FALSE, quote=FALSE)

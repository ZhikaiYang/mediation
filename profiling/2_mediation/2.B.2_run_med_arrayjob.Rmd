...
trait <- read.delim("data/blup_N_300_tax_groups.txt", header=T)
rfile = rfile[1:7] # remove P368
df <- data.frame(rnafile=rep(rfile, each=300), trait=rep(2:301, times=7))
write.table(df, "largedata/df_job_control_microbiome.csv", sep=",", row.names=FALSE, quote=FALSE)
...
```{r}
library("huskeR")
run_Rcode(inputdf=data.frame(file=1:2100, out=NA), outdir="slurm-script", cmdno = 10, 

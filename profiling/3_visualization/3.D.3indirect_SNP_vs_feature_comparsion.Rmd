...
isnps_gup5k_table$Var1 = as.character(isnps_gup5k_table$Var1)
...
pall <- ggplot(gup5k_overlap_eq, aes(x=tissue, y=o_ratio, fill=category)) +
    ...
    xlab("") +
    ylab("Overlap ratio with gup5k") +
    ggtitle("") +
    theme_classic() +
    labs(fill = "") +
    scale_y_continuous(limits = c(-0.01, max(gup5k_overlap_eq$o_ratio))) +
...
pdf("graphs/overlap_ratio_maxscale_gup5k_all_traits_eq.pdf", width = 9, height= 6)
pall
dev.off()
...
pall <- ggplot(gup5k_overlap_fixed, aes(x=tissue, y=o_ratio, fill=category)) +
    ...
    theme_classic() +
    labs(fill = "") +
    scale_y_continuous(limits = c(-0.01, max(gup5k_overlap_fixed$o_ratio))) +
    theme(plot.title = element_text(size=20, face = "bold"), 
          axis.text=element_text(size=16, face="bold"),
          strip.text.x = element_text(size = 16, face = "bold"),
          axis.title=element_text(size=fsize, face="bold"),
...
pdf("graphs/overlap_ratio_maxscale_gup5k_all_traits_fixed.pdf", width = 9, height= 6)
pall
dev.off()
...
#p7msnps_int327ear: 8181 0.01174508
#p7msnps_int34ear: 2631 0.003777204
#p7msnps_gup5k: 52416 0.06810014
...
#####change by features########################################################################################
gup5k_overlap_eq$p_f_null = 0.09975964
 
gup5k_overlap_fixed$p_f_null = 0.09975964
...
write.table(gup5k_overlap_eq, "largedata/isnps_vs_feature/gup5k_overlap_eq.csv", sep=",", row.names=FALSE, quote=FALSE)
write.table(gup5k_overlap_fixed, "largedata/isnps_vs_feature/gup5k_overlap_fixed.csv", sep=",", row.names=FALSE, quote=FALSE)

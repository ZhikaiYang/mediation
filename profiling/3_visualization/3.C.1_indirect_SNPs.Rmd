...
files <- list.files(path="largedata/med_output",
                   pattern="_indirect_",recursive = TRUE, full.names=T)

fmet <- list.files(path="largedata/med_output", pattern="_indirect_X\\.", full.names = T)
fmic <- list.files(path="largedata/med_output", pattern="_indirect_T[[0-9]", full.names = T)
id1 = which(files %in% fmet)
id2 = which(files %in% fmic)
...
out$file <- gsub(".*\\/", "", out$file)
out$trait <- gsub(".*indirect_", "", out$file)
out$trait <- gsub("_ts.*", "", out$trait)
...
table(out$tissue)
fwrite(out, "largedata/isnps_vs_feature/indirect_SNPs_agronomics.csv", sep=",", row.names=FALSE, quote=FALSE)
```
...
id <- fread("largedata/isnps_vs_feature/indirect_SNPs_agronomics.csv", header=TRUE, data.table=FALSE)
...
png("graphs/indirect_freq_plot_agronomics.png", width=11, height=4.25, units="in", res=300)
plot_mht(inputdf=tb, col2plot="Freq", jmph=FALSE, cl_file="data/chr_length_B73v4.csv", pl=TRUE,
         ylab="Indirect SNPs Freq",  cex = 0.6)
dev.off()
```
...
table(out$tissue)
fwrite(out, "largedata/isnps_vs_feature/indirect_SNPs_metabolites.csv", sep=",", row.names=FALSE, quote=FALSE)
```
...
id <- fread("largedata/isnps_vs_feature/indirect_SNPs_metabolites.csv", header=TRUE, data.table=FALSE)
id <- subset(id, method == "res.fixed")
id$trait <- gsub("\\.\\.s100.*", "", id$trait)
# Unique mediators
length(unique(id$medi)) #892
...
png("graphs/indirect_freq_plot_metabolites.png", width=11, height=4.25, units="in", res=300)
plot_mht(inputdf=tb, col2plot="Freq", jmph=FALSE, cl_file="data/chr_length_B73v4.csv", pl=TRUE,
         ylab="Indirect SNPs Freq",  cex = 0.6)
dev.off()
```

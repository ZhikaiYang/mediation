...
#####change by features########################################################################################
isnps_dmr <- fread("largedata/isnps_vs_feature/isnps_in_dmr.txt")
#####change by features########################################################################################


isnps_dmr_table <- as.data.frame(table(isnps_dmr$snps_for_medi)) 
isnps_dmr_table$Var1 = as.character(isnps_dmr_table$Var1)
isnps = merge(isnps, isnps_dmr_table, by.x = "snps_for_medi", by.y = "Var1", sort = F, all.x = T)
...
for (trait in i_trait_eq) {
  ...
  for (tissue in i_tissue_eq) {
    ...
    if (length(t) > 0) {
      ...
      dmr_overlap_eq = rbind(dmr_overlap_eq, c(trait, tissue, o_ratio, length(which(tem$Freq != 0)), nrow(tem) ))
    }
...
for (trait in i_trait_fixed) {
  ...
  for (tissue in i_tissue_fixed) {
    ...
    if (length(t) > 0) {
      tem = isnps_fixed[t, ]
      o_ratio = length(which(tem$Freq != 0))/nrow(tem)
      dmr_overlap_fixed = rbind(dmr_overlap_fixed, c(trait, tissue, o_ratio, length(which(tem$Freq != 0)), nrow(tem) ))
    }
...
dmr_overlap_eq = as.data.frame(dmr_overlap_eq)
colnames(dmr_overlap_eq) = c("trait", "tissue", "o_ratio", "num_o", "N_isnps")
dmr_overlap_fixed = as.data.frame(dmr_overlap_fixed)
colnames(dmr_overlap_fixed) = c("trait", "tissue", "o_ratio", "num_o", "N_isnps")

dmr_overlap_eq = merge(dmr_overlap_eq, trait_category, by = "trait", all.x = T, sort = F)
dmr_overlap_fixed = merge(dmr_overlap_fixed, trait_category, by = "trait", all.x = T, sort = F)
...
```{r}
fsize=18
dmr_overlap_eq$tissue <- factor(dmr_overlap_eq$tissue, levels = c( "GRoot", "GShoot", "L3Base", "L3Tip", "LMAD", "LMAN","Kern"),
...
dmr_overlap_eq$o_ratio = as.numeric(dmr_overlap_eq$o_ratio)

library(ggplot2)

#df0 <- subset(df, cat != "1")
pall <- ggplot(dmr_overlap_eq, aes(x=tissue, y=o_ratio, fill=category)) +
    ...
    xlab("") +
    ylab("Overlap ratio with DMR") +
    ggtitle("") +
    theme_classic() +
    labs(fill = "") +
    scale_y_continuous(limits = c(-0.01, max(dmr_overlap_eq$o_ratio))) +
...
pdf("graphs/overlap_ratio_maxscale_dmr_all_traits_eq.pdf", width = 9, height= 6)
pall
dev.off()
...
```{r}
fsize=18
dmr_overlap_fixed$tissue <- factor(dmr_overlap_fixed$tissue, levels = c( "GRoot", "GShoot", "L3Base", "L3Tip", "LMAD", "LMAN","Kern"),
                    labels =c( "Germinating Root", "Germinating Shoot", "Leaf 3 base", "Leaf 3 tip", "Adult leaf during the day", "Adult leaf at night","Kernels after pollination"))
dmr_overlap_fixed$o_ratio = as.numeric(dmr_overlap_fixed$o_ratio)

library(ggplot2)

#df0 <- subset(df, cat != "1")
pall <- ggplot(dmr_overlap_fixed, aes(x=tissue, y=o_ratio, fill=category)) +
    #geom_bar(stat="identity", position=position_dodge()) +
    geom_boxplot(outlier.colour="black", outlier.shape=16, 
             outlier.size=1, notch=FALSE) +
    # scale_fill_manual(values=c("#69b3a2", "#cdc0b0")) +
    ...
    theme_classic() +
    labs(fill = "") +
    scale_y_continuous(limits = c(-0.01, max(dmr_overlap_fixed$o_ratio))) +
    theme(plot.title = element_text(size=20, face = "bold"), 
          axis.text=element_text(size=16, face="bold"),
          strip.text.x = element_text(size = 16, face = "bold"),
          axis.title=element_text(size=fsize, face="bold"),
...
pdf("graphs/overlap_ratio_maxscale_dmr_all_traits_fixed.pdf", width = 9, height= 6)
pall
dev.off()
...
#p7msnps_dmr: 957 0.00137392
```{r}

#####change by features########################################################################################
dmr_overlap_eq$p_f_null = 0.00137392
 
dmr_overlap_fixed$p_f_null = 0.00137392

#####change by features########################################################################################



dmr_overlap_eq$num_o = as.numeric(dmr_overlap_eq$num_o)
dmr_overlap_fixed$num_o = as.numeric(dmr_overlap_fixed$num_o)
dmr_overlap_eq$N_isnps = as.numeric(dmr_overlap_eq$N_isnps)
dmr_overlap_fixed$N_isnps = as.numeric(dmr_overlap_fixed$N_isnps)

dmr_overlap_eq$p_val_chi = 0.0
dmr_overlap_eq$p_val_chi_adj = 0.0
dmr_overlap_eq$statistic_chi = 0.0
for (i in 1:nrow(dmr_overlap_eq)) {
  t = chisq.test(c(dmr_overlap_eq$num_o[i], dmr_overlap_eq$N_isnps[i]-dmr_overlap_eq$num_o[i]),p=c(dmr_overlap_eq$p_f_null[i], 1-dmr_overlap_eq$p_f_null[i]))
  dmr_overlap_eq$statistic_chi[i] = t$statistic
  dmr_overlap_eq$p_val_chi[i] = t$p.value
  dmr_overlap_eq$p_val_chi_adj[i] = p.adjust(dmr_overlap_eq$p_val_chi[i], method = "fdr", n= nrow(dmr_overlap_eq))
}

dmr_overlap_fixed$p_val_chi = 0.0
dmr_overlap_fixed$p_val_chi_adj = 0.0
dmr_overlap_fixed$statistic_chi = 0.0
for (i in 1:nrow(dmr_overlap_fixed)) {
  t = chisq.test(c(dmr_overlap_fixed$num_o[i], dmr_overlap_fixed$N_isnps[i]-dmr_overlap_fixed$num_o[i]),p=c(dmr_overlap_fixed$p_f_null[i], 1-dmr_overlap_fixed$p_f_null[i]))
  dmr_overlap_fixed$statistic_chi[i] = t$statistic
  dmr_overlap_fixed$p_val_chi[i] = t$p.value
  dmr_overlap_fixed$p_val_chi_adj[i] = p.adjust(dmr_overlap_fixed$p_val_chi[i], method = "fdr", n= nrow(dmr_overlap_fixed))
}

length(which(dmr_overlap_eq$p_val_chi_adj > 0.05))
length(which(dmr_overlap_fixed$p_val_chi_adj > 0.05))



hist(dmr_overlap_eq$p_val_chi_adj[- which(dmr_overlap_eq$p_val_chi_adj > 0.05)], main = "Distribution of adjusted p-value by FDR (Medfix.0.5)", xlab = "adjusted p-value")

hist(dmr_overlap_fixed$p_val_chi_adj[- which(dmr_overlap_fixed$p_val_chi_adj > 0.05)], main = "Distribution of adjusted p-value by FDR (Medfix.bic)", xlab = "adjusted p-value")

write.table(dmr_overlap_eq, "largedata/isnps_vs_feature/dmr_overlap_eq.csv", sep=",", row.names=FALSE, quote=FALSE)
write.table(dmr_overlap_fixed, "largedata/isnps_vs_feature/dmr_overlap_fixed.csv", sep=",", row.names=FALSE, quote=FALSE)
...
ggplot(data=df_nonsig, aes(x=window_size, y=count_fixed, fill=non_sig)) +
  geom_bar(stat="identity")+
  geom_text(aes(y=label_ypos_fixed, label=count_fixed), vjust=1.6, 
            color="white", size=3.5)+
  scale_fill_brewer(palette="Paired")+

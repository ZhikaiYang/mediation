...
```{r}
TPall = fread("largedata/simulation/TPall_h2.txt", sep = "\t",header = T)
...
plist2 = ggplot(TPall_stat[which((TPall_stat$qtl == qtli)&(TPall_stat$qtlar == qtlari)),], aes(x=top, y=True_positive_rate, group=h2, color=h2)) + 
    geom_errorbar(aes(ymin=True_positive_rate-sd, ymax=True_positive_rate+sd), width=.1, 
    position=position_dodge(0.05)) +
    geom_line() + geom_point()+
    theme_minimal()+
...
library(ggpubr)
figure <- ggarrange(p1, p2, p3,p4,p5,p6,p7,p8,p9, ncol = 3, nrow = 3)
figure
```

...
###' functions from SKAT package, with minor modification
...
Get_PValue.Lambda<-function(lambda,Q){
  ...
  for(i in 1:n1){
    out<-davies(Q[i],lambda,acc=10^(-6))  ## call the function from CompQuadForm package directly. SKAT includes a copy of this function in the package
...
Get_Davies_PVal.Lambda<-function(Q, lambda, Q.resampling = NULL){ ## function for calculating p-value for SKAT using Davis' method
  
  #  K<-W/2
  
  Q.all<-c(Q,Q.resampling)
  
  re<-Get_PValue.Lambda(lambda,Q.all)
  param<-list()
  ...
  re<-list(p.value = re$p.value[1], param=param,p.value.resampling = p.value.resampling, pval.zero.msg=re$pval.zero.msg )  
  return(re)
}

...
###sample 20000 out of 0.7M new geno
plink -bfile hmp321_282_agpv4_maf005_miss03_pruned_sam20000 --freq --missing --out hmp321_282_agpv4_maf005_miss03_pruned_sam20000
...
###sample 20000 out of 0.7M new geno
maf <- fread("largedata/geno/hmp321_282_agpv4_maf005_miss03_pruned_sam20000.frq", header=TRUE)
lmiss <- fread("largedata/geno/hmp321_282_agpv4_maf005_miss03_pruned_sam20000.lmiss", header=TRUE)


pdf("largedata/geno/hmp321_282_agpv4_maf005_miss03_pruned_sam20000_maf_lmiss.pdf", width = 10, height=5)
par(mfrow=c(1,2))
hist(maf$MAF, breaks=100, col="#cdb79e", main="MAF (sample 20,000 out of 769,690 SNPs)", xlab="Minor Allele Freq")
abline(v=0.01, lty=2, col="black", lwd=3)
abline(v=0.05, lty=2, col="red", lwd=3)

hist(lmiss$F_MISS, breaks=100, col="#cdb79e", main="Missingness (sample 20,000 out of 769,690 SNPs)", xlab="Missing Rate")
abline(v=0.6, lty=2, col="red", lwd=3)
abline(v=0.2, lty=2, col="red", lwd=3)
dev.off()
...
###sample 20000 out of 0.7M new geno
plink --bfile hmp321_282_agpv4_maf005_miss03_pruned_sam20000  --ld-window 10 --ld-window-kb 100 --r2 --ld-window-r2 0  --out hmp321_282_agpv4_maf005_miss03_pruned_sam20000
...
ld <- fread("largedata/geno/hmp321_282_agpv4_maf005_miss03_pruned_sam20000.ld", data.table=FALSE)
ld$dis <- ld$BP_B - ld$BP_A
# summarize for each bp distance, the mean R2 
ld <- as.data.table(ld)

df <- ld[,.(meanr2=mean(R2), sd=sd(R2)), by=dis]
write.table(df, "largedata/geno/hmp321_282_agpv4_maf005_miss03_pruned_sam20000_ld_mean_sd.csv", sep=",", quote=FALSE)

df <- read.csv("largedata/geno/hmp321_282_agpv4_maf005_miss03_pruned_sam20000_ld_mean_sd.csv")
library(ggplot2)

pdf("largedata/geno/hmp321_282_agpv4_maf005_miss03_pruned_sam20000_LD_decay.pdf", width = 10, height=5)
par(mfrow=c(1,2))
p1 <- ggplot(df, aes(x=dis, y=meanr2)) +
    #facet_grid(. ~ type) +
    geom_point(size=2) +
...
#scatterplot3d(iris[,1:3], pch = shapes)
sub_pop <- read.delim("largedata/geno/sub_pop_282.csv", sep = ",",header=T)
sub_pop = sub_pop[,c(1,9)]
df_sub_pop = merge(df, sub_pop, by.x = "V2", by.y = "Inbred", all.x = T)
df_sub_pop = df_sub_pop[,c(1,3,4,5,23)]
df_sub_pop$Subpopulation = as.factor(df_sub_pop$Subpopulation)
